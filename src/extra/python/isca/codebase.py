...
class SocratesCodeBase(CodeBase):
    ...
    def simlink_to_soc_code(self, socrates_version_to_use):
        #Make symlink to socrates source code if one doesn't already exist.
        socrates_desired_location = self.codedir+'/src/atmos_param/socrates/src/trunk/'+socrates_version_to_use
        ...
        if os.path.exists(socrates_desired_location):
            ...
            if link_correct:
                ...
                if os.path.islink(socrates_desired_location):
                    self.log.info('Socrates '+socrates_version_to_use+' source code symlink is in correct place, but is to incorrect location. Trying to correct.')
                    os.unlink(socrates_desired_location)
                else:
                    self.log.info('Socrates '+socrates_version_to_use+' source code is in correct place, but folder structure is wrong. Contents of the folder '+socrates_desired_location+' should include a src folder.')
        else:
            socrates_code_in_desired_location=False
            self.log.info('Socrates '+socrates_version_to_use+' source code symlink does not exist. Creating.')
        ...
        if socrates_code_in_desired_location:
            self.log.info('Socrates '+socrates_version_to_use+' source code already in correct place. Continuing.')
        else:
            if GFDL_SOC is not None:
                soc_simlink_loc = GFDL_SOC+'/'+socrates_version_to_use+'/trunk'
                sh.ln('-s', soc_simlink_loc, socrates_desired_location)
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.simlink_to_soc_code(socrates_version_to_use)
