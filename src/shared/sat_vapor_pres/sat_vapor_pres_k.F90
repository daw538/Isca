...
! benefit and could be done as a second step anyway, if necessary.
 ...
 subroutine sat_vapor_pres_init_k(table_size, tcmin, tcmax, TFREEZE, TFREEZE_H2O, HLV, &
                                  RVGAS, TPPRESS, ES0, err_msg, &
...
! This routine has been generalized to return tables for any temperature range and resolution
 ...
 real, intent(in) :: TFREEZE, TFREEZE_H2O, HLV, RVGAS, TPPRESS, ES0
...
! increment used to generate derivative table
  ...
  integer :: i
   ...
   if (construct_table_wrt_liq_and_ice) then
      ...
      tminl = real(tcmin)+tfreeze_h2o  ! minimum valid temp in table. 
      ! conversion from K to deg C so tfreeze_h2o required
      dtinvl = 1./dtres
      tepsl = .5*dtres
      tinrc = .1*dtres
