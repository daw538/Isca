...
use fms_mod, only: write_version_number, file_exist, close_file, stdlog, error_mesg, NOTE, FATAL, WARNING, read_data, field_size, uppercase, mpp_pe, check_nml_error

! cp_air needed for rrtmg and pstd_mks needed for pref calculation
use           constants_mod, only: grav, rdgas, rvgas, cp_air, PSTD_MKS, dens_vapor
...
if(do_bm .and. do_llcs) &
  call error_mesg('idealized_moist_phys','do_bm and do_llcs cannot both be .true.',FATAL)
...
allocate(depth_change_lh(is:ie, js:je))    ; depth_change_lh = 0.0
allocate(depth_change_cond(is:ie, js:je))  ; depth_change_cond = 0.0
allocate(depth_change_conv(is:ie, js:je))  ; depth_change_conv = 0.0
...
case(LLCS_CONV)
  ...
  do k_llcs = 1, size(ug,3)
    ...
    p_full_llcs(:,:,size(ug,3)-k_llcs+1) = p_half(:,:,k_llcs,previous)
  end do
  ...
  precip = rain ! rain from LLCS is already in kgm^-2s^-1, no need to
                ! divide by delta_t
