...
!   INTEGER :: id_soc_surf_spectrum_sw !not implemented yet
  ...
  integer :: id_rrsun, id_dec, id_ang, id_true_anom, id_time_since_ae ! Orbital Diagnostics
  
  REAL(r_def) :: ang_out_store, dec_out_store, true_anomaly_store, rrsun_store, time_since_ae_store
...
!         'watts/m2', missing_value=missing_value               )
    ...
    IF (js == 1) THEN
       ...
       if ((account_for_effect_of_water .eqv. .false.) .and. (do_condensate_ch4 .eqv. .true.)) then
  ...
  subroutine socrates_interface(Time_diag, rlat, rlon, soc_lw_mode,  &
    ...
    real(r_def), dimension(n_profile, n_layer) :: input_p, input_t, input_mixing_ratio, &
         ...
         z_full_reshaped, &
         input_cld_frac, input_reff_rad, input_mmr_cl_rad, input_cld_conv_frac   
    ...
    sk = size(fms_temp,3)
          ...
          if ((account_for_effect_of_water .eqv. .true.) .and. (do_condensate_ch4 .eqv. .true.)) then
              call error_mesg('socrates_interface','Cannot set water and methane as condensing species simultaneously.',FATAL)
          elseif ((account_for_effect_of_water .eqv. .true.) .and. (do_condensate_ch4 .eqv. .false.)) then
          ...
          elseif ((account_for_effect_of_water .eqv. .false.) .and. (do_condensate_ch4 .eqv. .true.)) then
    ...
    type(time_type) :: Time_loc
        ...
        ! alarm
          ...
          if(r_total_seconds - dt_last .ge. dt_rad) then
            ...
            if (id_dec > 0) then
                used = send_data ( id_dec, dec_out, Time_diag)
            endif            
            if (id_ang > 0) then
       ...
       elseif (frierson_solar_rad) then
        ...
        ! compute zenith angle
           ...
           if(do_rad_time_avg) then
	         ...
	         call diurnal_solar(rad_lat, rad_lon, gmt, time_since_ae, coszen, fracsun, rrsun, dt_rad_radians, true_anom=true_anomaly, dec_out=dec_out, ang_out=ang_out)
           else
	         ! Seasonal Cycle: Use astronomical parameters to calculate insolation
	         call diurnal_solar(rad_lat, rad_lon, gmt, time_since_ae, coszen, fracsun, rrsun, true_anom=true_anomaly, dec_out=dec_out, ang_out=ang_out)
           end if
